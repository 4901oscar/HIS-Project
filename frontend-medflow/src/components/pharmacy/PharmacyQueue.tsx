    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <div className="text-center py-12 text-gray-400">
          ...
          <p className="text-sm font-medium">No hay recetas pendientes de despacho</p>
          <p className="text-xs mt-1">Las recetas pagadas aparecerán aquí</p>
        </div>
      </div>
    );
  }
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-left text-xs text-gray-500 uppercase tracking-wide">
              ...
              <th className="pb-2 pr-4">Código Receta</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {sortedAppointments.map(appt => (
              <tr key={appt.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="py-3 pr-4 font-mono text-xs font-medium text-green-700">
                  {/* Prescription code will be loaded when viewing details */}
                  —
                </td>
