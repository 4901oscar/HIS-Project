...
public class EmailService {
    ...
    public void sendTempPasswordEmail(String toEmail, String firstName,
        ...
        send(toEmail, "Bienvenido a MedFlow HIS â€” Credenciales de acceso",
    ...
    public void sendEmployeeCreatedEmail(String toEmail, String firstName,
                                         String username, String tempPassword) {
        send(toEmail, "Cuenta de empleado creada â€” MedFlow HIS",
    ...
    public void sendAppointmentConfirmationEmail(String toEmail, String firstName,
        ...
        send(toEmail, "ConfirmaciÃ³n de Cita â€” MedFlow HIS",
    ...
    private void send(String to, String subject, String htmlBody) {
        try {
            ...
            log.info("[Email] Enviado a {} â€” {}", to, subject);
        } catch (Exception e) {
    ...
    private String buildActivationHtml(String firstName, String link) {
        return "<!DOCTYPE html><html lang=\"es\"><head><meta charset=\"UTF-8\"></head><body style=\"margin:0;padding:0;background:#f4f6f8;font-family:Arial,sans-serif;\">"
            ...
            + "<p style=\"margin:4px 0 0;color:#b0d4e8;font-size:12px;\">Sistema de InformaciÃ³n Hospitalaria</p></td></tr>"
            + "<tr><td style=\"padding:36px 40px;\">"
            + "<h2 style=\"margin:0 0 16px;color:#1a1a2e;font-size:20px;\">Hola, " + firstName + " ðŸ‘‹</h2>"
            + "<p style=\"color:#444;line-height:1.6;margin:0 0 24px;\">Gracias por registrarte en MedFlow HIS. Para activar tu cuenta haz clic en el botÃ³n:</p>"
            ...
            + "<p style=\"color:#888;font-size:12px;margin:0 0 8px;\">El enlace es vÃ¡lido por <strong>24 horas</strong>. Si no creaste esta cuenta, ignora este correo.</p>"
            + "<p style=\"color:#bbb;font-size:11px;margin:0;\">Si el botÃ³n no funciona copia este enlace:<br><span style=\"color:#00d4e8;\">" + link + "</span></p>"
            + "</td></tr>"
            + "<tr><td style=\"background:#f4f6f8;padding:16px 40px;text-align:center;\"><p style=\"margin:0;color:#aaa;font-size:11px;\">&copy; 2025 MedFlow HIS &middot; Correo automÃ¡tico.</p></td></tr>"
    ...
    private String buildTempPasswordHtml(String firstName, String username,
        ...
        return "<!DOCTYPE html><html lang=\"es\"><head><meta charset=\"UTF-8\"></head><body style=\"margin:0;padding:0;background:#f4f6f8;font-family:Arial,sans-serif;\">"
            ...
            + "<tr><td style=\"padding:0 20px 14px;\"><p style=\"margin:0 0 6px;color:#555;font-size:13px;\"><strong>ContraseÃ±a temporal:</strong></p>"
            ...
            + "<p style=\"color:#e74c3c;font-size:13px;margin:0 0 24px;\">âš ï¸ Cambia tu contraseÃ±a en el primer inicio de sesiÃ³n.</p>"
            + "<div style=\"text-align:center;\"><a href=\"" + loginUrl + "\" style=\"display:inline-block;background:#00d4e8;color:#0f4c75;font-weight:bold;font-size:15px;padding:14px 32px;border-radius:6px;text-decoration:none;\">Iniciar sesiÃ³n</a></div>"
            + "</td></tr>"
            + "<tr><td style=\"background:#f4f6f8;padding:16px 40px;text-align:center;\"><p style=\"margin:0;color:#aaa;font-size:11px;\">&copy; 2025 MedFlow HIS &middot; Correo automÃ¡tico.</p></td></tr>"
            + "</table></td></tr></table></body></html>";
    }
    ...
    private String buildEmployeeHtml(String firstName, String username,
        ...
        return "<!DOCTYPE html><html lang=\"es\"><head><meta charset=\"UTF-8\"></head><body style=\"margin:0;padding:0;background:#f4f6f8;font-family:Arial,sans-serif;\">"
            ...
            + "<tr><td style=\"padding:0 20px 14px;\"><p style=\"margin:0 0 6px;color:#555;font-size:13px;\"><strong>ContraseÃ±a temporal:</strong></p>"
            + "<p style=\"margin:0;font-family:monospace;font-size:20px;color:#0f4c75;font-weight:bold;letter-spacing:2px;\">" + tempPassword + "</p></td></tr></table>"
            + "<p style=\"color:#e74c3c;font-size:13px;margin:0 0 24px;\">âš ï¸ Cambia tu contraseÃ±a en tu primer inicio de sesiÃ³n.</p>"
    ...
    private String buildAppointmentConfirmationHtml(String firstName, String appointmentDate,
        ...
        return "<!DOCTYPE html><html lang=\"es\"><head><meta charset=\"UTF-8\"></head>"
            ...
            + "<p style=\"margin:4px 0 0;color:#b0d4e8;font-size:12px;\">Sistema de InformaciÃ³n Hospitalaria</p></td></tr>"
            + "<tr><td style=\"padding:36px 40px;\">"
            + "<h2 style=\"margin:0 0 16px;color:#1a1a2e;font-size:20px;\">Â¡Cita confirmada, " + firstName + "! ðŸŽ‰</h2>"
            + "<p style=\"color:#444;line-height:1.6;margin:0 0 24px;\">Tu cita ha sido agendada exitosamente. A continuaciÃ³n los detalles:</p>"
            + "<table style=\"background:#f0faff;border:1px solid #b8e4f0;border-radius:6px;width:100%;margin:0 0 24px;\">"
            + "<tr><td style=\"padding:14px 20px;\"><p style=\"margin:0 0 6px;color:#555;font-size:13px;\"><strong>ðŸ“… Fecha:</strong></p>"
            + "<p style=\"margin:0;font-size:16px;color:#0f4c75;font-weight:bold;\">" + appointmentDate + "</p></td></tr>"
            + "<tr><td style=\"padding:0 20px 14px;\"><p style=\"margin:0 0 6px;color:#555;font-size:13px;\"><strong>ðŸ• Hora:</strong></p>"
            + "<p style=\"margin:0;font-size:16px;color:#0f4c75;font-weight:bold;\">" + appointmentTime + "</p></td></tr>"
            + "<tr><td style=\"padding:0 20px 14px;\"><p style=\"margin:0 0 6px;color:#555;font-size:13px;\"><strong>ðŸ‘¨â€âš•ï¸ Doctor:</strong></p>"
            + "<p style=\"margin:0;font-size:16px;color:#0f4c75;font-weight:bold;\">" + doctorName + "</p></td></tr>"
            + "<tr><td style=\"padding:0 20px 14px;\"><p style=\"margin:0 0 6px;color:#555;font-size:13px;\"><strong>ðŸ“„ Factura:</strong></p>"
            + "<p style=\"margin:0;font-family:monospace;font-size:14px;color:#0f4c75;font-weight:bold;\">" + invoiceNumber + "</p></td></tr>"
            + "</table>"
            + notesSection
            + "<div style=\"text-align:center;margin:24px 0;\">"
            + "<p style=\"color:#555;font-size:14px;margin:0 0 12px;\"><strong>Tu cÃ³digo QR de confirmaciÃ³n:</strong></p>"
            ...
            + "<p style=\"color:#e74c3c;font-size:13px;margin:12px 0 0;\">â° QR vÃ¡lido desde <strong>" + validFromTime + "</strong> hasta <strong>" + validUntilTime + "</strong></p>"
            + "<p style=\"color:#888;font-size:12px;margin:8px 0 0;\">Presenta este cÃ³digo en recepciÃ³n el dÃ­a de tu cita</p>"
            + "</div>"
            + "</td></tr>"
            + "<tr><td style=\"background:#f4f6f8;padding:16px 40px;text-align:center;\">"
            + "<p style=\"margin:0;color:#aaa;font-size:11px;\">&copy; 2025 MedFlow HIS &middot; Correo automÃ¡tico.</p>"
            + "</td></tr>"
            + "</table></td></tr></table></body></html>";
    }
}
